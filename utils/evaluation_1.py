...
def evaluate_multi_cls(y_true, y_pred, y_proba, print_conf=True, text_file=None, class_names=None, loss=0, lr=None):
    ...
    g_score = geometric_mean_score(y_true, y_pred, average='macro')
    ...
    if print_conf:
        if text_file is not None:
            if lr is None:
                print("K={:.2f} - BACC={:.2f} - MCC={:.2f} - F1={:.2f} - G_score={:.2f} - AUC={:.2f} - Loss={:.4f}".format(100*k, 100*bacc, 100*mcc, 100*f1, 100*g_score, 100*mean_auc, loss),
                      end=" ", file=open(text_file, "a"))
            else:
                print("K={:.2f} - BACC={:.2f} - MCC={:.2f} - F1={:.2f} - G_score={:.2f} - AUC={:.2f} - Loss={:.4f} - LR={:.5f}".format(100*k, 100*bacc, 100*mcc, 100*f1, 100*g_score, 100*mean_auc, loss, lr),
                    end=" ", file=open(text_file, "a"))
            ...
            if len(class_names)==3:
                ...
                return mean_auc, k, mcc, f1, g_score, bacc, auc_all, f1_all, acc_all
