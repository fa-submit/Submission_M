...
def get_sampling_probabilities(class_count, mode='instance', ep=None, n_eps=None):
    ...
    if mode == 'natural':
        sampling_probabilities = relative_freq
    else: 
        sampling_probabilities = relative_freq ** (-1)
    return sampling_probabilities

def get_sampling_probabilities_1(metric, mode='instance', ep=None, n_eps=None):
    '''
    Note that for progressive sampling I use n_eps-1, which I find more intuitive.
    If you are training for 10 epochs, you pass n_eps=10 to this function. Then, inside
    the training loop you would have sth like 'for ep in range(n_eps)', so ep=0,...,9,
    ...
    relative_freq = (1-np.array(metric)) / (1-np.array(metric)).sum()
    # print(relative_freq)
    sampling_probabilities = relative_freq #** (-1)
...
def modify_loader_1(loader, metric, mode, ep=None, n_eps=None):
    sampling_probs = get_sampling_probabilities_1(metric, mode=mode, ep=ep, n_eps=n_eps)
    print(metric)
...
def get_combo_loader(loader, metric, base_sampling='instance'):
    # if base_sampling == 'instance':
    #     imbalanced_loader = loader
    # else:
    # print(metric)
    imbalanced_loader = modify_loader_1(loader, metric, mode=base_sampling)
    print(base_sampling)
    balanced_loader = modify_loader(loader, mode=base_sampling)
...
def get_train_val_cls_datasets(csv_path_train, csv_path_val, data_path=None, mean=None, std=None, tg_size=(512, 512), see_classes=True):
    ...
    if see_classes:
        print(20 * '*')
        print(np.unique(train_dataset.dr))
